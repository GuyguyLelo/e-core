(function ($) {
    ...
    try {
      ...
      if (
        $simplebarWrapper.length > 0 &&
        $("#pageWrapper").hasClass("compact-wrapper")
      ) {
        var $activeLink = $simplebarWrapper.find("a.active");
        if ($activeLink.length > 0) {
          var offset = $activeLink.offset();
          if (offset && typeof offset !== 'undefined' && typeof offset.top !== 'undefined' && !isNaN(offset.top)) {
      ...
      console.warn("Error scrolling to active link:", e);
    }
  })(jQuery);